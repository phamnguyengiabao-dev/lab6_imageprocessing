...
for epoch in range(epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        # Original train function takes single batch X, y
        # NN.train(X, y_onehot, learning_rate) 
        # But wait, the original `train` method does one forward and one backward pass. 
        # It's manual GD.
...
# 7. Prediction
print("Testing prediction on one sample...")
images, labels = next(iter(test_loader))
x_predict = images[0]
label = labels[0].item()

# Scale/Normalize if needed? MNIST is 0-1 already if ToTensor used.

output = NN.forward(x_predict.unsqueeze(0)) # Add batch dim 1x784
predicted_label = torch.argmax(output).item()

print(f"True Label: {label}")
print(f"Predicted Label: {predicted_label}")
print(f"Output Probabilities: {output.detach().numpy()}")
